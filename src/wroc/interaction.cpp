    ...
    if (base_event.type == wroc_event_type::pointer_motion
        ...
        if (auto* toplevel = movesize.grabbed_toplevel.get()) {
            if (server->interaction_mode == wroc_interaction_mode::move) {
                toplevel->base->position = movesize.surface_grab + vec2i32(glm::round(event.pointer->layout_position - movesize.pointer_grab));
            } else if (server->interaction_mode == wroc_interaction_mode::size) {
                auto new_size = glm::max(movesize.surface_grab + vec2i32(glm::round(event.pointer->layout_position - movesize.pointer_grab)), vec2i32{});
