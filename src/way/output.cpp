...
WAY_BIND_GLOBAL(wl_output, bind)
{
    ...
    way_send(bind.server, wl_output_send_geometry, resource,
        size.x, size.y,
        0, 0,
    ...
    way_send(bind.server, wl_output_send_mode, resource,
        WL_OUTPUT_MODE_CURRENT | WL_OUTPUT_MODE_PREFERRED,
        size.x, size.y,
        0);

    if (bind.version >= WL_OUTPUT_SCALE_SINCE_VERSION) {
        way_send(bind.server, wl_output_send_scale, resource, 1);
    }

    if (bind.version >= WL_OUTPUT_NAME_SINCE_VERSION) {
        way_send(bind.server, wl_output_send_name, resource, "ROC-1");
    }

    if (bind.version >= WL_OUTPUT_DESCRIPTION_SINCE_VERSION) {
        way_send(bind.server, wl_output_send_description, resource, "unknown");
    }

    if (bind.version >= WL_OUTPUT_DONE_SINCE_VERSION) {
        way_send(bind.server, wl_output_send_done, resource);
    }
}
